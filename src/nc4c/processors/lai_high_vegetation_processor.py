...
from nc4c.data_models.lai_high_vegetation import (
    LAI_HV_VARIABLE,
    calculate_lai_high_vegetation,
)
...
class LAI_High_VegetationProcessor(BaseDataProcessor):
    """高植被叶面积指数图像生成处理器"""
    ...
    def process(self, dataset: xr.Dataset) -> xr.DataArray:
        """获取叶面积指数"""
        return calculate_lai_high_vegetation(
            dataset=dataset,
            variables=LAI_HV_VARIABLE,
        )
