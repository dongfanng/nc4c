...
from nc4c.data_models.lai_low_vegetation import (
    LAI_LV_VARIABLE,
    calculate_lai_low_vegetation,
)
...
class LAI_Low_VegetationProcessor(BaseDataProcessor):
    """低植被叶面积指数图像生成处理器"""
    ...
    def process(self, dataset: xr.Dataset) -> xr.DataArray:
        """获取叶面积指数"""
        return calculate_lai_low_vegetation(
            dataset=dataset,
            variables=LAI_LV_VARIABLE,
        )
