...
from nc4c.data_models.surface_net_solar_radiation import (
    VARIABLE_NAME,
    calculate_net_solar_radiation,
)
...
class SurfaceNetSolarRadiationProcessor(BaseDataProcessor):
    """表面净短波辐射图像生成处理器"""
    ...
    def process(self, dataset: xr.Dataset) -> xr.DataArray:
        """计算表面净短波辐射"""
        return calculate_net_solar_radiation(
            dataset=dataset,
