...
from nc4c.data_models.soil_moisture import SWVL1_VARIABLE, calculate_soil_moisture
...
class SoilMoistureProcessor(BaseDataProcessor):
    """土壤水分层1图像生成处理器"""
    ...
    def process(self, dataset: xr.Dataset) -> xr.DataArray:
        """处理土壤水分数据"""
        return calculate_soil_moisture(
            dataset=dataset,
            variables=SWVL1_VARIABLE,
        )
